...
def test_calibration_weather_is_continuous_and_distinguishes_provisional_day():
    data = Path(__file__).parents[1] / "data/calibration/tres_arroyos_2026_weather.csv"
    frame = pd.read_csv(data)
    assert pd.to_datetime(frame.Fecha).tolist() == pd.date_range("2026-01-01", "2026-09-16").tolist()
    assert frame.TipoDato.value_counts().to_dict() == {"Observado": 258, "Provisional": 1}
    assert frame.loc[frame.TipoDato.eq("Provisional"), "Fecha"].tolist() == ["2026-09-16"]
    assert frame.loc[frame.TipoDato.eq("Observado"), "Fuente"].eq("SIGA_INTA_TRES_ARROYOS_BARROW").all()
