...
def saved_profile():
    return load_site_profile(DATA / "tres_arroyos_2026.json")


def apply(frame=None, **kwargs):
    arguments = {
        "site": "Tres Arroyos", "as_of": "2027-01-08",
...
def test_invalid_profile_cannot_change_outputs():
    ...
    with pytest.raises(ValueError, match="finitos"):
        apply_site_calibration(example_trajectory(), profile, site="Tres Arroyos", as_of="2027-01-08")
...
def real_data():
    ...
    raw = pd.read_csv(DATA / "tres_arroyos_2026_counts.csv")
...
def test_source_units_missing_replicates_and_irregular_intervals(real_data):
    ...
    assert len(raw) == 17 and "n_repeticiones" not in metadata
    assert raw.columns.tolist() == ["FECHA", "PLM2"]
    assert raw.PLM2.sum() == pytest.approx(12089.666666666668)
    _, intervals = calibration_intervals(trajectory, prepared)
    assert intervals.iloc[0]["Inicio_exclusivo"] == pd.Timestamp("2026-02-05")
    assert intervals.iloc[0]["Fecha"] == pd.Timestamp("2026-02-12")
    assert intervals.iloc[0]["Dias_intervalo"] == 7
    assert intervals.iloc[0]["Observado_PLM2"] == pytest.approx(0.)
    assert intervals.loc[intervals.Fecha.eq("2026-09-01"), "Dias_intervalo"].iloc[0] == 29
    ...
    assert np.allclose(intervals.Sigma_ajuste_PLM2, 910.5333333333333)
    assert intervals.Observado_PLM2.sum() == pytest.approx(raw.iloc[:, -1].sum())


def test_calibration_keeps_tres_arroyos_decay_and_local_reference(real_data):
    trajectory, _, _, _ = real_data
    result, audit = apply(trajectory, as_of="2026-09-16")
    assert audit["applied"]
    ...
    saved = saved_profile()
    assert saved["model_parameters"]["cobertura_pct"] == 20.
...
def test_fit_matches_persisted_profile_and_does_not_mutate_network(real_data):
    ...
    profile, comparison = fit_site_calibration(trajectory, prepared, site="Tres Arroyos")
    saved = saved_profile()
...
def test_invalid_training_data_are_rejected(real_data, fault):
    ...
    with pytest.raises(ValueError):
        fit_site_calibration(trajectory, obs, site="Tres Arroyos")
