"""Interfaz PREDWEEM Digital Twin — Tres Arroyos."""
...
st.markdown(
    ...
    <div class="hero">
      ...
      <h1>Gemelo Digital · Lolium Tres Arroyos</h1>
...
try:
    calibration_profile = load_site_profile(CALIBRATION_DIR / "tres_arroyos_2026.json")
...
if source_option == "SIGA Barrow + ECMWF operativa":
    st.caption(
        "Meteorología: observaciones SIGA–INTA Barrow, puente histórico ECMWF "
...
with tab_state:
    ...
    st.toggle(
        ...
        help=(
            ...
            "El perfil de Tres Arroyos es experimental; su aplicación depende de la "
...
with tab_calibration:
    st.subheader("Memoria local de Tres Arroyos · campaña 2026")
    st.info(calibration_audit["reason"])
    if calibration_profile is not None:
        fit = calibration_profile["fit"]
        source = calibration_profile["source"]
        fit_table = pd.read_csv(CALIBRATION_DIR / "tres_arroyos_2026_fit.csv")
        st.write(
        ...
        st.dataframe(
            pd.read_csv(CALIBRATION_DIR / "tres_arroyos_2026_holdout.csv"),
        ...
        st.download_button(
            ...
            (CALIBRATION_DIR / "tres_arroyos_2026_counts.csv").read_bytes(),
            "tres_arroyos_2026_counts.csv", "text/csv",
        )
        st.caption(
    ...
    audit_summary = pd.DataFrame(
        {
            ...
            "Valor": [
                ...
                "Tres Arroyos vK4.9.15 Visual V3",
                (
